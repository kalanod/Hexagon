...
int Game::autoTurnFirst(int deep) {
    ...
    for (int i = 0; i < tmp.size(); i++) {
        ...
        if (tmp[i] == (turn % 2) + 1 && false) {
            if (board.canMove(i, i + 10, turn) != 0) {
                tmpBoard = board.copy();
                if (turn % 2) {
                    b = tmpBoard.getPerl();
                } else {
